...
class ApiClientSpec : RobolectricBaseSpec() {
    ...
    fun `should include all query parameter`() {
        ...
        val client = ApiClient(baseUrl, mockContext)

        enqueueResponse("test-body")
        server.enqueue(MockResponse().setResponseCode(304))
        val response = client.fetchPath("test-path", paramMap)

        for ((k, v) in paramMap) response.request.url.queryParameter(k) shouldBeEqualTo v
    }
    ...
    fun `should include have empty value query param`() {
        ...
        response.request.url.queryParameterNames.size shouldBeEqualTo 1
        response.request.url.queryParameter("key1") shouldBeEqualTo "value1"
    ...
    private fun createClient(url: String = baseUrl) =
        ApiClient(url, ApplicationProvider.getApplicationContext())
}
